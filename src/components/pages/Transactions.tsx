...
export const Transactions: React.FC = () => {
  const { transactions, employees, attendanceRecords, attendanceShifts } = useData();
  ...
  const handleAutomateAttendance = () => {
    ...
    // Calc Absence
    const daysInMonth = records.reduce((acc, r) => {
      const d = r.timestamp.split('T')[0];
      acc.add(d);
      return acc;
    }, new Set<string>());

    // This is a simple logic: if it's a workday and no records, it's absence
    // For simplicity, we assume 22-26 work days depending on selection
    // But let's just count how many days he was PRESENT
    const presentDaysCount = daysInMonth.size;
    const estimatedWorkDays = 30; // Standard month
    const derivedAbsenceDays = Math.max(0, 30 - presentDaysCount - 4); // assuming 4 fridays
    ...
    // Convert minutes to deduction value (simple logic: 1 hour = 1 hour salary)
    ...
    alert('تم جلب بيانات الحضور بنجاح');
  };
              ...
              <form onSubmit={handleSubmit} className="p-8 space-y-6 max-h-[70vh] overflow-y-auto">
                <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
                  ...
                  <div className="space-y-2">
                    ...
                    <input type="number" className="w-full px-5 py-3 bg-gray-50 border border-gray-100 rounded-2xl focus:ring-2 focus:ring-blue-500 outline-none font-medium" value={formData.actualWorkDays || 0} onChange={(e) => setFormData({...formData, actualWorkDays: Number(e.target.value) || 0})} />
                  </div>
                  <div className="space-y-2">
                    <label className="text-sm font-bold text-gray-500 mr-2">ساعات العمل في اليوم</label>
                    <input type="number" className="w-full px-5 py-3 bg-gray-50 border border-gray-100 rounded-2xl focus:ring-2 focus:ring-blue-500 outline-none font-medium" value={formData.dailyWorkHours || 0} onChange={(e) => setFormData({...formData, dailyWorkHours: Number(e.target.value) || 0})} />
                  </div>
                  ...
                  <div className="space-y-2">
                    ...
                    <input type="number" className="w-full px-5 py-3 bg-gray-50 border border-gray-100 rounded-2xl focus:ring-2 focus:ring-blue-500 outline-none font-medium" value={formData.basicSalary || 0} onChange={(e) => setFormData({...formData, basicSalary: Number(e.target.value) || 0})} />
                  </div>
                  <div className="space-y-2">
                    <label className="text-sm font-bold text-gray-500 mr-2">بدل سكن</label>
                    <input type="number" className="w-full px-5 py-3 bg-gray-50 border border-gray-100 rounded-2xl focus:ring-2 focus:ring-blue-500 outline-none font-medium" value={formData.housingAllowance || 0} onChange={(e) => setFormData({...formData, housingAllowance: Number(e.target.value) || 0})} />
                  </div>
                  <div className="space-y-2">
                    <label className="text-sm font-bold text-gray-500 mr-2">بدل نقل</label>
                    <input type="number" className="w-full px-5 py-3 bg-gray-50 border border-gray-100 rounded-2xl focus:ring-2 focus:ring-blue-500 outline-none font-medium" value={formData.transportAllowance || 0} onChange={(e) => setFormData({...formData, transportAllowance: Number(e.target.value) || 0})} />
                  </div>
                  <div className="space-y-2">
                    <label className="text-sm font-bold text-gray-500 mr-2">بدل إعاشة</label>
                    <input type="number" className="w-full px-5 py-3 bg-gray-50 border border-gray-100 rounded-2xl focus:ring-2 focus:ring-blue-500 outline-none font-medium" value={formData.subsistenceAllowance || 0} onChange={(e) => setFormData({...formData, subsistenceAllowance: Number(e.target.value) || 0})} />
                  </div>
                  <div className="space-y-2">
                    <label className="text-sm font-bold text-gray-500 mr-2">بدل جوال</label>
                    <input type="number" className="w-full px-5 py-3 bg-gray-50 border border-gray-100 rounded-2xl focus:ring-2 focus:ring-blue-500 outline-none font-medium" value={formData.mobileAllowance || 0} onChange={(e) => setFormData({...formData, mobileAllowance: Number(e.target.value) || 0})} />
                  </div>
                  <div className="space-y-2">
                    <label className="text-sm font-bold text-gray-500 mr-2">بدل إدارة</label>
                    <input type="number" className="w-full px-5 py-3 bg-gray-50 border border-gray-100 rounded-2xl focus:ring-2 focus:ring-blue-500 outline-none font-medium" value={formData.managementAllowance || 0} onChange={(e) => setFormData({...formData, managementAllowance: Number(e.target.value) || 0})} />
                  </div>
                  <div className="space-y-2">
                    <label className="text-sm font-bold text-gray-500 mr-2">بدلات أخرى</label>
                    <input type="number" className="w-full px-5 py-3 bg-gray-50 border border-gray-100 rounded-2xl focus:ring-2 focus:ring-blue-500 outline-none font-medium" value={formData.otherAllowances || 0} onChange={(e) => setFormData({...formData, otherAllowances: Number(e.target.value) || 0})} />
                  </div>
                  <div className="space-y-2">
                    <label className="text-sm font-bold text-gray-500 mr-2">ساعات الإضافي</label>
                    <input type="number" className="w-full px-5 py-3 bg-gray-50 border border-gray-100 rounded-2xl focus:ring-2 focus:ring-blue-500 outline-none font-medium" value={formData.overtimeHours || 0} onChange={(e) => setFormData({...formData, overtimeHours: Number(e.target.value) || 0})} />
                  </div>
                  <div className="space-y-2">
                    <label className="text-sm font-bold text-gray-500 mr-2">قيمة الإضافي</label>
                    <input type="number" className="w-full px-5 py-3 bg-gray-50 border border-gray-100 rounded-2xl focus:ring-2 focus:ring-blue-500 outline-none font-medium text-emerald-600 bg-emerald-50/50" value={formData.overtimeValue || 0} onChange={(e) => setFormData({...formData, overtimeValue: Number(e.target.value) || 0})} />
                  </div>
                  <div className="space-y-2">
                    <label className="text-sm font-bold text-gray-500 mr-2">زيادة راتب</label>
                    <input type="number" className="w-full px-5 py-3 bg-gray-50 border border-gray-100 rounded-2xl focus:ring-2 focus:ring-blue-500 outline-none font-medium" value={formData.salaryIncrease || 0} onChange={(e) => setFormData({...formData, salaryIncrease: Number(e.target.value) || 0})} />
                  </div>
                  <div className="space-y-2">
                    <label className="text-sm font-bold text-gray-500 mr-2">دخل آخر</label>
                    <input type="number" className="w-full px-5 py-3 bg-gray-50 border border-gray-100 rounded-2xl focus:ring-2 focus:ring-blue-500 outline-none font-medium" value={formData.otherIncome || 0} onChange={(e) => setFormData({...formData, otherIncome: Number(e.target.value) || 0})} />
                  </div>
                  ...
                  <div className="space-y-2">
                    ...
                    <input type="number" className="w-full px-5 py-3 bg-gray-50 border border-gray-100 rounded-2xl focus:ring-2 focus:ring-blue-500 outline-none font-medium" value={formData.socialInsurance || 0} onChange={(e) => setFormData({...formData, socialInsurance: Number(e.target.value) || 0})} />
                  </div>
                  <div className="space-y-2">
                    <label className="text-sm font-bold text-gray-500 mr-2">استلام راتب</label>
                    <input type="number" className="w-full px-5 py-3 bg-gray-50 border border-gray-100 rounded-2xl focus:ring-2 focus:ring-blue-500 outline-none font-medium" value={formData.salaryReceived || 0} onChange={(e) => setFormData({...formData, salaryReceived: Number(e.target.value) || 0})} />
                  </div>
                  <div className="space-y-2">
                    <label className="text-sm font-bold text-gray-500 mr-2">استلام بنك</label>
                    <input type="number" className="w-full px-5 py-3 bg-gray-50 border border-gray-100 rounded-2xl focus:ring-2 focus:ring-blue-500 outline-none font-medium" value={formData.bankReceived || 0} onChange={(e) => setFormData({...formData, bankReceived: Number(e.target.value) || 0})} />
                  </div>
                  <div className="space-y-2">
                    <label className="text-sm font-bold text-gray-500 mr-2">سلف</label>
                    <input type="number" className="w-full px-5 py-3 bg-gray-50 border border-gray-100 rounded-2xl focus:ring-2 focus:ring-blue-500 outline-none font-medium" value={formData.loans || 0} onChange={(e) => setFormData({...formData, loans: Number(e.target.value) || 0})} />
                  </div>
                  <div className="space-y-2">
                    <label className="text-sm font-bold text-gray-500 mr-2">أيام الغياب</label>
                    <div className="flex gap-2">
                       <input type="number" className="flex-1 px-5 py-3 bg-gray-50 border border-gray-100 rounded-2xl focus:ring-2 focus:ring-blue-500 outline-none font-medium" value={formData.absenceDays || 0} onChange={(e) => setFormData({...formData, absenceDays: Number(e.target.value) || 0})} />
                       <button 
                         type="button" 
                         disabled={isCalculating || !formData.employeeId}
                         onClick={handleAutomateAttendance}
                         className="px-4 bg-teal-50 text-teal-600 rounded-xl hover:bg-teal-100 transition-all text-xs font-black disabled:opacity-50"
                       >
                         {isCalculating ? 'جاري الحساب...' : 'جلب بيانات الحضور'}
                       </button>
                    </div>
                  </div>
                  <div className="space-y-2">
                    <label className="text-sm font-bold text-gray-500 mr-2">خصم الغياب</label>
                    <input type="number" className="w-full px-5 py-3 bg-gray-50 border border-gray-100 rounded-2xl focus:ring-2 focus:ring-blue-500 outline-none font-medium text-red-600 bg-red-50/50" value={formData.absenceDeduction || 0} onChange={(e) => setFormData({...formData, absenceDeduction: Number(e.target.value) || 0})} />
                  </div>
                  <div className="space-y-2">
                    <label className="text-sm font-bold text-gray-500 mr-2">ساعات الخصم</label>
                    <input type="number" className="w-full px-5 py-3 bg-gray-50 border border-gray-100 rounded-2xl focus:ring-2 focus:ring-blue-500 outline-none font-medium" value={formData.deductionHours || 0} onChange={(e) => setFormData({...formData, deductionHours: Number(e.target.value) || 0})} />
                  </div>
                  <div className="space-y-2">
                    <label className="text-sm font-bold text-gray-500 mr-2">تأخير ومغادرات</label>
                    <input type="number" className="w-full px-5 py-3 bg-gray-50 border border-gray-100 rounded-2xl focus:ring-2 focus:ring-blue-500 outline-none font-medium" value={formData.departureDelayDeduction || 0} onChange={(e) => setFormData({...formData, departureDelayDeduction: Number(e.target.value) || 0})} />
                  </div>
                  <div className="space-y-2">
                    <label className="text-sm font-bold text-gray-500 mr-2">خصومات أخرى</label>
                    <input type="number" className="w-full px-5 py-3 bg-gray-50 border border-gray-100 rounded-2xl focus:ring-2 focus:ring-blue-500 outline-none font-medium" value={formData.otherDeductions || 0} onChange={(e) => setFormData({...formData, otherDeductions: Number(e.target.value) || 0})} />
                  </div>

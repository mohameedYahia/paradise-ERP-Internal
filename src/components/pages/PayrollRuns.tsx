...
export const PayrollRuns: React.FC = () => {
  ...
  const calculatePayroll = async () => {
    ...
    const mNumber = parseInt(month.split('-')[1]);
    ...
    const results: PayrollResult[] = employees
      .map(emp => {
        ...
        let missionAllowance = 0;

        // Automated Logic: Iterate through month days
        for (let d = 1; d <= lastDay; d++) {
          ...
          // Check manual absence
          const customAbsence = allAbsences.find(a => 
          ...
          if (hasAttendance) {
            ...
            // Daily allowances are added for every day of the mission
            const dailySum = (mission.allowances || []).filter(a => a.type === 'Daily').reduce((sum, a) => sum + a.amount, 0);
            missionAllowance += dailySum;
            
            // Once allowances are added only on the first day of the mission (if in this month)
            if (dateStr === mission.startDate) {
              const onceSum = (mission.allowances || []).filter(a => a.type === 'Once').reduce((sum, a) => sum + a.amount, 0);
              missionAllowance += onceSum;
            }
          } else if (customAbsence) {
            const aType = absenceTypes.find(at => at.id === customAbsence.absenceTypeId);
            if (aType) autoAbsenceDays += aType.deductionRatio;
          } else {
            // Automatic Absence
            autoAbsenceDays++;
          }
        ...
        const details = calculatePayrollDetails({
          ...
          absenceDays: (empTrans?.absenceDays || 0) + autoAbsenceDays,
          missionAllowance: (empTrans?.missionAllowance || 0) + missionAllowance,
